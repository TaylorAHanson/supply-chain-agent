...
def create_volumes():
    ...
    print(f"Creating volume {schema}.skills...")
    execute_sql(f"CREATE VOLUME IF NOT EXISTS {schema}.skills")
    execute_sql(f"GRANT READ VOLUME ON VOLUME {schema}.skills TO `{sp_client_id}`")
    execute_sql(f"GRANT WRITE VOLUME ON VOLUME {schema}.skills TO `{sp_client_id}`")
